        ...
        try {
            ...
            try {
                ...
                if (user) {
                    ...
                    if (!isNumber(user.tikus)) user.tikus = 0
                    if (!isNumber(user.bebek)) user.bebek = 0
                    if (!isNumber(user.merak)) user.merak = 0
                    if (!isNumber(user.flaminggo)) user.flaminggo = 0
                    if (!isNumber(user.monyet)) user.monyet = 0
                    if (!isNumber(user.cacing)) user.cacing = 0
                    if (!isNumber(user.tupai)) user.tupai = 0
                    if (!isNumber(user.bakso)) user.bakso = 0
                    if (!isNumber(user.seblak)) user.seblak = 0
                    if (!isNumber(user.naspad)) user.naspad = 0
                    if (!isNumber(user.sate)) user.sate = 0
                    if (!isNumber(user.sayur)) user.sayur = 0
                    if (!isNumber(user.capcai)) user.capcai = 0
                    if (!isNumber(user.ayambakar)) user.ayambakar = 0
                    if (!isNumber(user.pindang)) user.pindang = 0
                    if (!isNumber(user.tahu)) user.tahu = 0
                    if (!isNumber(user.pecel)) user.pecel = 0
                    if (!isNumber(user.geprek)) user.geprek = 0
                    if (!isNumber(user.martabak)) user.martabak = 0
                    if (!isNumber(user.rusa)) user.rusa = 0
                    if (!isNumber(user.merpati)) user.merpati = 0                
                    if (!isNumber(user.tikus)) user.tikus = 0
                    ...
                    if (!isNumber(user.lastmit)) user.lastmit = 0
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                     //   this.reply(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}buy*`, m)
                        this.sendButton(m.chat, `Limit anda habis, silahkan beli melalui */buyall* atau */hadiah*`, author, null, [['Buy Limit', '/buyall'], ['Hadiah', '/hadiah']], m)
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        premium: 'Kamu Tidak Premium SILAHKAN hubungan owner Jika Ingin premium.', 
